...
Crx2Rnx::Crx2Rnx(char _pathCrx[])
{
    char *argCrx2Rnx[2] = {"Crx2Rnx", _pathCrx};
...
Crx2Rnx::Crx2Rnx()
{
//    char *_pathCrx = new char[str.size() + 1];
//    strcpy(_pathCrx, str.c_str());
...
bool Crx2Rnx::unCompr(std::string str)
{
    bool b;
    char *_pathCrx = new char[str.size() + 1];
    strcpy(_pathCrx, str.c_str());
...
bool Crx2Rnx::go(int argc, char *argv[])
{
    ...
    while( fgets(dline,MAXCLM,stdin) != NULL ){      /*** exit program successfully ***/
        ...
        if(crinex_version == 3) { /*** skip escape lines of CRINEX version 3 ***/
            while(dline[0] == '&'){
                nl_count++;
                if( fgets(dline,MAXCLM,stdin) == NULL ) return true;//return exit_status;
                CHOP_LF(dline,p);
            }
        }
        if(!exitStatus){
        ...
        if(dline[0] == ep_top_from){
            ...
            if(*p_event!='0' && *p_event!='1' ){
                if(put_event_data(dline,p_event)!=0) {skip_to_next(dline); if(!exitStatus) return false;} //redaction
                goto SKIP;
            }
            line[0] = '\0';          /**** initialize arc for epoch data ***/
            nsat1 = 0;               /**** initialize the all satellite arcs ****/
        }else if( dline[0] == '\032' ){
            return true;//return exit_status;   /** DOS EOF **/
        }
        ...
        if(line[0] != ep_top_to || strlen(line)<(26+offset) || *(p+23) != ' '
                             || *(p+24) != ' ' || ! isdigit(*(p+25)) ) {
            skip_to_next(dline);
            if(!exitStatus) return false; //redaction
            goto SKIP;
        }
        CHOP_BLANK(line,p);
        ...
        for(i=0,i0=sattbl ; i<nsat ; i++,i0++){
            ...
            if( getdiff(y_1[i],y_0[*i0],*i0,dflag[i]) != 0 ) {
                skip_to_next(dline);
                if(!exitStatus) return false; //redaction
                goto SKIP;
            }
...
void Crx2Rnx::fileopen(int argc, char *argv[]){
    ...
    if(nfout == 0) {
        ...
        if((freopen(outfile,"r",stdout)) != NULL && force == 0){
            ...
            if(getchar() != 'y') { return; } //exit(EXIT_SUCCESS);  };
        }
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::read_clock(char *dline, long *yu, long *yl){
    char *p,*s,*p1;
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::process_clock(char *dline){
    int i,j;
    /****************************************/
...
/*---------------------------------------------------------------------*/
int  Crx2Rnx::put_event_data(char *dline, char *p_event){
/***********************************************************************/
...
/***********************************************************************/
    ...
    do {
        ...
        do {
            nl_count++;
            if(fgets(dline,MAXCLM,stdin) == NULL) return 1;  //exit(exit_status);  /*** eof: exit program successfully ***/
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::skip_to_next(char *dline){
    char *p;
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::set_sat_table(char *p_new, char *p_old, int nsat1, int *sattbl){
/***********************************************************************/
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::data(char *p_sat_lst, int *sattbl, char dflag[][MAXTYPE*2]){
/********************************************************************/
/*  Functions                                                       */
...
/********************************************************************/
    ...
    for(i=0,i0=sattbl,p=p_sat_lst ; i<nsat ; i++,i0++,p+=3){
        ...
        /**** ---------------------------- ****/
        for(j=0,py1=y_1[i] ; j<ntype ; j++,py1++){
            if(py1->arc_order >= 0){
                py0 = &(y_0[*i0][j]);
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::repair(char *s, char *ds){
    for(; *s != '\0' && *ds != '\0' ; ds++,s++){
        if(*ds == ' ')continue;
        if(*ds == '&')
            *s = ' ';
...
/*---------------------------------------------------------------------*/
int  Crx2Rnx::getdiff(data_format *y, data_format *y0, int i0, char *dflag){
    int j,length;
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::putfield(data_format *y, char *flag){
    int  i;
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::print_clock(long yu, long yl, int shift_clk){
    char tmp[8],*p_tmp,*p;
    int n,sgn;
...
/*---------------------------------------------------------------------*/
int  Crx2Rnx::read_chk_line(char *line){
    char *p;
...
/*---------------------------------------------------------------------*/
void Crx2Rnx::error_exit(int error_no, char *string){
    exitStatus = false;
    if(error_no == 1 ){
