...
Rnx2Crx::Rnx2Crx(char _pathRnx[])
{
    char *argv[2] = {"Rnx2Crx", _pathRnx};
    qDebug()<<"Rnx2Crx: "<<QDir::currentPath();
    go(2, argv);
}

Rnx2Crx::Rnx2Crx()
{
//    char *_pathRnx = new char[rnx_path.size() + 1];
//    strcpy(_pathRnx, rnx_path.c_str());

//    char *argCrx2Rnx[2] = {"Crx2Rnx", _pathRnx};
//    qDebug()<<"Crx2Rnx: "<<QDir::currentPath();

//    go(2, argCrx2Rnx);
//    delete [] _pathRnx;
}

bool Rnx2Crx::compr(std::string rnx_path)
{
    bool b;
    char *_pathRnx = new char[rnx_path.size() + 1];
    strcpy(_pathRnx, rnx_path.c_str());
...
bool Rnx2Crx::go(int argc, char *argv[])
{
    ...
    for(CLEAR_BUFF;;FLUSH_BUFF){
        ...
        if( ! get_next_epoch(newline) ){
            std::fflush(stdout);
            return true;//return exit_status;
        ...
        if(nsat > 12 && rinex_version == 2) read_more_sat(nsat,p_satlst);  /*** read continuation lines ***/ //2 RINEX VERSION
        if( ep_reset > 0 && ++ep_count > ep_reset ) initialize_all(oldline,&nsat_old,1);

        /**** get observation ****/
        for(i=0,p=p_satlst ; i<nsat ; i++,p+=3) {
            if( ggetline(y_1[i],flag[i],p,&ntype_record[i]) ) {
                CLEAR_BUFF;
                exit_status = false;//exit_status = EXIT_WARNING;
                goto SKIP;
            }
        }
        *p='\0';    /*** terminate satellite list ***/
        ...
        for(i=0;i<nsat;i++){
            ...
            for(j=0;j<ntype_record[i];j++) y_0[i][j] = y_1[i][j];
        }
        if(!exitStatus)
            return false;
    }
...
void Rnx2Crx::parse_args(int argc, char *argv[]){
...
/*---------------------------------------------------------------------*/
int  Rnx2Crx::get_next_epoch(char *p_line){
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::skip_to_next(char *p_line){
    int nchar;
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::initialize_all(char *oldline,int *nsat_old,int count){
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::put_event_data(char *p_line){
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::read_clock(char *p_clock,int shift_clk){
...
/*---------------------------------------------------------------------*/
int  Rnx2Crx::set_sat_table(char *p_new, char *p_old, int nsat_old,int *sattbl){
...
/*---------------------------------------------------------------------*/
int  Rnx2Crx::read_more_sat(int n, char *p){
...
/*---------------------------------------------------------------------*/
char *Rnx2Crx::strdiff(char *s1, char *s2, char *ds){
/********************************************************************/
...
/*---------------------------------------------------------------------*/
int  Rnx2Crx::ggetline(data_format *py1, char *flag, char *sat_id, int *ntype_rec){
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::read_value(char *p, long *pu, long *pl){
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::take_diff(data_format *py1, data_format *py0){
    int k;
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::putdiff(long dddu, long dddl){
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::put_clock(long du, long dl, int c_order){
/***********************************/
...
/*---------------------------------------------------------------------*/
int  Rnx2Crx::read_chk_line(char *line){
    char *p;
/***************************************/
...
/*---------------------------------------------------------------------*/
void Rnx2Crx::error_exit(int error_no, char *string){
    exitStatus = false;
    if(error_no == 1 ){
